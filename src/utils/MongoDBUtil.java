...
public enum MongoDBUtil {
    ...
    static {
        ...
        options.connectionsPerHost(1000);// 连接池设置为1000个连接,默认为100
        options.connectTimeout(15000);// 连接超时，推荐>3000毫秒
        options.maxWaitTime(5000); //
        options.socketTimeout(0);// 套接字超时时间，0无限制
        options.threadsAllowedToBlockForConnectionMultiplier(10000);// 线程队列数，如果连接线程排满了队列就会抛出“Out of semaphores to get db”错误。
    ...
    public static void main(String[] args) {
    	try {
//    		String xmlRoot = LoadProperties.getValue("xmlRoot");
    		String dbName = "test";
    		...
    		FileFilter fileFilter = new FileFilter() {
    			public boolean accept(File pathname) {
    				...
    				return tmp.matches(".*?\\.xml");
    			}
    		};
    		System.out.println(coll);
//    		File root = new File(xmlRoot);
//    		File[] resultsFile = root.listFiles(fileFilter);
    		//String regex = "srml-(\\d+)-(\\d+)-results\\.xml";
    		//Pattern p = Pattern.compile(regex);
    		
    		/*for(File file : resultsFile){
    			System.out.println(file.getName()+"---"+new SimpleDateFormat("yyyyMMddHHmmss").format(new Date(file.lastModified())));
    			//Matcher m = p.matcher(file.getName());
    			//if(m.find()){
    			//	System.out.println("compId:"+m.group(1)+",season:"+m.group(2));
    				Document doc = new Document();
    				String id = file.getName();
    				doc.put("_id", id);
    				//doc.put("cId", m.group(1));
    				//doc.put("season", m.group(2));
    				doc.put("lastModify", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date(file.lastModified())));
    				doc.put("timestamp", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
			...
			//doc.put("cId", m.group(1));
			//doc.put("season", m.group(2));
			doc.put("lastModify", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
			doc.put("timestamp", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
//			coll.insertOne(doc);
//	         Document docs= MongoDBUtil.instance.findById(coll, id);
	         Document dd=getMatch("20161215165205"); 
...
//         }
         ...
         // 根据ID查询
//         String id = "5628701cb86969119cbb758c";
//         Document doc = MongoDBUtil.instance.findById(coll, id);
//         System.out.println(doc);
//        
//         //查询多个
//         MongoCursor<Document> cursor1 = coll.find(Filters.eq("field_id", 2635)).iterator();
...
//        MongoDBUtil.instance.find(coll, filter);
    ...
    public static Document getMatch(String mid){
		try {
			MongoCollection<Document> coll = MongoDBUtil.instance.getCollection("test", "test");
			MongoCursor<Document> cursor = coll.find(new BasicDBObject("lastModify", mid)).iterator();
